    ...
    @IBAction func btnPostQ(_ sender: UIButton) {
        ...
        if isTitleEmpty == false && isTfQEmpty == false {
            ...
            let okAction = UIAlertAction(title: "확인", style: .default) { (action) in // ok 누를 시 실행할 함수 입력
                // FU : 서버로 tfFirstQ, tfSecondQ의 두 가지 정보 전달, 서버로 tfTitleBox의 글 정보 전달, 서버로 익명질문여부(isAnonymous) 정보 전달, FU : 서버로 질문검색여부(isSearchQ) 정보 전달
                self.presentingViewController?.dismiss(animated: true)
                }
            let noAction = UIAlertAction(title: "취소", style: .default) { (action) in }
            
            agreeNoChangeAlert.addAction(noAction)
            agreeNoChangeAlert.addAction(okAction)
